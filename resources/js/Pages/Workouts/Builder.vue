...
const props = defineProps({
    workout:   Object,   // null = new
    paceZones: Object,   // { z1: { min_pace, max_pace }, ... }
});
...
const BLOCK_DEFS = [
    {
        type: 'warmup',   label: 'Aufwärmen',   zone: 2,
        color: 'green',   barColor: '#22c55e',
    ...
    {
        type: 'ramp_up',  label: 'Ramp up',     zone: 3,
        color: 'blue',    barColor: '#3b82f6',
        hasRampSteps: true,
    ...
    {
        type: 'active',   label: 'Hauptblock',  zone: 3,
        color: 'amber',   barColor: '#f97316',
    ...
    {
        type: 'repeat',   label: 'Wiederholung', zone: 4,
        color: 'red',     barColor: '#ef4444',
        hasRepeatSteps: true,
    ...
    {
        type: 'ramp_down', label: 'Ramp down',   zone: 2,
        color: 'indigo',   barColor: '#6366f1',
        hasRampSteps: true,
    ...
    {
        type: 'recovery',  label: 'Erholung',    zone: 1,
        color: 'teal',     barColor: '#14b8a6',
        icon: `<path stroke-linecap="round" stroke-linejoin="round" d="M21 8.25c0-2.485-2.099-4.5-4.688-4.5-1.935 0-3.597 1.126-4.312 2.733-.715-1.607-2.377-2.733-4.313-2.733C5.1 3.75 3 5.765 3 8.25c0 7.22 9 12 9 12s9-4.78 9-12Z" />`,
    },
    {
        type: 'cooldown',  label: 'Auslaufen',   zone: 2,
        color: 'slate',    barColor: '#64748b',
    ...
    {
        type: 'rest',      label: 'Pause',       zone: 0,
        color: 'gray',     barColor: '#d1d5db',
        icon: `<path stroke-linecap="round" stroke-linejoin="round" d="M15.75 5.25v13.5m-7.5-13.5v13.5" />`,
    },
    {
        type: 'free',      label: 'Frei',        zone: 2,
        color: 'purple',   barColor: '#a855f7',
...
const ZONE_COLORS = {
    0: { border: 'border-gray-200 dark:border-slate-700', badge: 'bg-gray-100 dark:bg-slate-700 text-gray-600 dark:text-slate-300', bar: '#94a3b8' },
    1: { border: 'border-gray-200 dark:border-slate-700', badge: 'bg-gray-100 dark:bg-slate-700 text-gray-600 dark:text-slate-300', bar: '#94a3b8' },
    2: { border: 'border-green-200 dark:border-green-700/50', badge: 'bg-green-100 dark:bg-green-500/20 text-green-700 dark:text-green-400', bar: '#22c55e' },
    3: { border: 'border-amber-200 dark:border-amber-700/50', badge: 'bg-amber-100 dark:bg-amber-500/20 text-amber-700 dark:text-amber-400', bar: '#f97316' },
    4: { border: 'border-orange-200 dark:border-orange-700/50', badge: 'bg-orange-100 dark:bg-orange-500/20 text-orange-700 dark:text-orange-400', bar: '#f97316' },
    5: { border: 'border-red-200 dark:border-red-700/50', badge: 'bg-red-100 dark:bg-red-500/20 text-red-700 dark:text-red-400', bar: '#ef4444' },
};

const BLOCK_COLOR_CLASSES = {
    green:  { stripe: 'bg-green-500',  btn: 'bg-green-50 dark:bg-green-500/10 text-green-700 dark:text-green-400 border-green-200 dark:border-green-500/30' },
    blue:   { stripe: 'bg-blue-500',   btn: 'bg-blue-50 dark:bg-blue-500/10 text-blue-700 dark:text-blue-400 border-blue-200 dark:border-blue-500/30' },
    amber:  { stripe: 'bg-amber-500',  btn: 'bg-amber-50 dark:bg-amber-500/10 text-amber-700 dark:text-amber-400 border-amber-200 dark:border-amber-500/30' },
    ...
    indigo: { stripe: 'bg-indigo-500', btn: 'bg-indigo-50 dark:bg-indigo-500/10 text-indigo-700 dark:text-indigo-400 border-indigo-200 dark:border-indigo-500/30' },
    teal:   { stripe: 'bg-teal-500',   btn: 'bg-teal-50 dark:bg-teal-500/10 text-teal-700 dark:text-teal-400 border-teal-200 dark:border-teal-500/30' },
    slate:  { stripe: 'bg-slate-400',  btn: 'bg-slate-50 dark:bg-slate-500/10 text-slate-600 dark:text-slate-400 border-slate-200 dark:border-slate-500/30' },
    ...
    purple: { stripe: 'bg-purple-500', btn: 'bg-purple-50 dark:bg-purple-500/10 text-purple-700 dark:text-purple-400 border-purple-200 dark:border-purple-500/30' },
};

function defFor(type) {
    return BLOCK_DEFS.find(d => d.type === type) ?? BLOCK_DEFS[2];
}
...
function addBlock(def) {
    ...
    const block = {
        ...
        label:        def.label,
        duration_mode:'time',
        duration_sec: def.type === 'rest' ? 120 : 600,
        distance_m:   null,
        pace_zone:    def.zone,
        pace:         '',
    };
    ...
    if (def.hasRampSteps) {
        block.steps = def.type === 'ramp_down'
            ? [{ zone: 4, duration_sec: 120 }, { zone: 3, duration_sec: 120 }, { zone: 2, duration_sec: 120 }, { zone: 1, duration_sec: 120 }]
            : [{ zone: 1, duration_sec: 120 }, { zone: 2, duration_sec: 120 }, { zone: 3, duration_sec: 120 }, { zone: 4, duration_sec: 120 }];
        delete block.duration_sec;
        delete block.distance_m;
        delete block.duration_mode;
        delete block.pace_zone;
        delete block.pace;
    }
...
function addRampStep(block) {
    ...
    block.steps.push({ zone: Math.min(5, lastZone + 1), duration_sec: 120 });
}
function removeRampStep(block, idx) { if (block.steps.length > 2) block.steps.splice(idx, 1); }

// ── Repeat sub-steps ─────────────────────────────────────────────────────────
function addRepeatStep(block) {
    block.steps.push({ _id: uid(), type: 'rest', label: 'Schritt', duration_mode: 'distance', distance_m: 200, duration_sec: null, pace_zone: 1, pace: '' });
}
...
const barSegments = computed(() => {
    ...
    function addSeg(zoneOrColor, w) {
        if (w <= 0) return;
        const color = typeof zoneOrColor === 'string' ? zoneOrColor : (ZONE_COLORS[zoneOrColor]?.bar ?? '#94a3b8');
        if (segments.length && segments.at(-1).color === color) {
            segments.at(-1).weight += w;
        } else {
            segments.push({ color, weight: w });
        }
        totalW += w;
    }
    ...
    for (const b of blocks.value) {
        ...
        if (b.type === 'repeat') {
            ...
            for (const sub of b.steps ?? []) {
                const w = sub.duration_mode === 'distance' ? (sub.distance_m || 0) : (sub.duration_sec || 0);
                addSeg(sub.pace_zone ?? 3, w * reps);
            }
        } else if (b.type === 'ramp_up' || b.type === 'ramp_down') {
            for (const step of b.steps ?? []) {
                addSeg(step.zone ?? 2, step.duration_sec ?? 120);
            }
        } else {
            const w = b.duration_mode === 'distance' ? (b.distance_m || 0) : (b.duration_sec || 0);
            addSeg(b.pace_zone ?? def.zone, w);
        }
    }

    if (!totalW) return [];
    return segments.map(s => ({ ...s, pct: Math.round(s.weight / totalW * 100) })).filter(s => s.pct > 0);
});
...
async function save() {
    ...
    try {
        ...
        if (isEdit.value) {
            await axios.put(route('workouts.update', props.workout.id), payload);
        } else {
            await axios.post(route('workouts.store'), payload);
        }
            ...
            <div class="grid lg:grid-cols-3 gap-5">
                ...
                <div class="space-y-4">
                    ...
                    <div class="bg-white dark:bg-slate-900 rounded-2xl border border-gray-100 dark:border-slate-800 p-4 space-y-3">
                        <div>
                            <label class="block text-xs font-semibold text-gray-500 dark:text-slate-400 uppercase tracking-wide mb-1">Name</label>
                        ...
                        <div>
                            <label class="block text-xs font-semibold text-gray-500 dark:text-slate-400 uppercase tracking-wide mb-1">Typ</label>
                            <select v-model="type"
                                class="w-full rounded-xl border border-gray-200 dark:border-slate-700 bg-gray-50 dark:bg-slate-800 px-3 py-2 text-sm text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-indigo-500">
                        ...
                        <div>
                            <label class="block text-xs font-semibold text-gray-500 dark:text-slate-400 uppercase tracking-wide mb-1">Notiz</label>
                        ...
                        <div>
                            <label class="block text-xs font-semibold text-gray-500 dark:text-slate-400 uppercase tracking-wide mb-1">Tags <span class="font-normal normal-case">(mit Leerzeichen trennen)</span></label>
                    ...
                    <div class="bg-white dark:bg-slate-900 rounded-2xl border border-gray-100 dark:border-slate-800 p-4">
                        <p class="text-xs font-semibold text-gray-500 dark:text-slate-400 uppercase tracking-wide mb-3">Segment hinzufügen</p>
                        <div class="grid grid-cols-3 gap-2">
                            <button v-for="def in BLOCK_DEFS" :key="def.type"
                                @click="addBlock(def)"
                                class="flex flex-col items-center gap-1 rounded-xl border px-2 py-2.5 text-[11px] font-semibold transition-colors hover:shadow-sm"
                                :class="BLOCK_COLOR_CLASSES[def.color].btn">
                                <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.8" v-html="def.icon" />
                ...
                <div class="lg:col-span-2 space-y-4">
                    ...
                    <div class="bg-white dark:bg-slate-900 rounded-2xl border border-gray-100 dark:border-slate-800 p-4">
                        <!-- Pace zone bar -->
                        <div class="h-3 rounded-full overflow-hidden flex mb-3 bg-gray-100 dark:bg-slate-800">
                            <div v-for="(seg, i) in barSegments" :key="i"
                                :style="{ width: seg.pct + '%', backgroundColor: seg.color }"
                                class="transition-all duration-300" />
                            <div v-if="!barSegments.length" class="w-full bg-gray-200 dark:bg-slate-700 rounded-full" />
                        </div>
                        <div class="flex gap-6 text-sm">
                            <div>
                                <span class="text-lg font-bold text-gray-900 dark:text-white">{{ totalDistM ? fmtDist(totalDistM) : '—' }}</span>
                                <span class="text-xs text-gray-400 dark:text-slate-500 ml-1">gesamt</span>
                            </div>
                            <div>
                                <span class="text-lg font-bold text-gray-900 dark:text-white">{{ totalDurSec ? fmtDur(totalDurSec) : '—' }}</span>
                                <span class="text-xs text-gray-400 dark:text-slate-500 ml-1">Zeit</span>
                            </div>
                            <div class="ml-auto text-xs text-gray-400 dark:text-slate-500 self-center">{{ blocks.length }} Segment{{ blocks.length !== 1 ? 'e' : '' }}</div>
                        </div>
                    </div>
                        ...
                        <div v-for="(block, idx) in blocks" :key="block._id ?? idx"
                            ...
                            <div class="flex items-center gap-2 px-3 py-2.5 cursor-pointer select-none"
                                ...
                                <div class="flex-1 min-w-0">
                                    ...
                                    <span class="text-xs text-gray-400 dark:text-slate-500 ml-2">
                                        ...
                                        <template v-else-if="block.duration_mode === 'distance' && block.distance_m">{{ (block.distance_m/1000).toFixed(block.distance_m >= 1000 ? 2 : 0) }} {{ block.distance_m >= 1000 ? 'km' : 'm' }}</template>
                                        <template v-else-if="block.duration_sec">{{ fmtDur(block.duration_sec) }}</template>
                                    </span>
                                    <span v-if="block.pace_zone > 0" class="text-[11px] ml-2 px-1.5 py-0.5 rounded-full"
                                ...
                                <div class="flex gap-0.5 shrink-0" @click.stop>
                                    <button @click="moveUp(idx)" :disabled="idx === 0" class="p-1 rounded text-gray-300 dark:text-slate-600 hover:text-gray-500 dark:hover:text-slate-400 disabled:opacity-20 transition-colors">
                                    ...
                                    <button @click="moveDown(idx)" :disabled="idx === blocks.length - 1" class="p-1 rounded text-gray-300 dark:text-slate-600 hover:text-gray-500 dark:hover:text-slate-400 disabled:opacity-20 transition-colors">
                                    ...
                                    <button @click="duplicateBlock(idx)" class="p-1 rounded text-gray-300 dark:text-slate-600 hover:text-gray-500 dark:hover:text-slate-400 transition-colors">
                                    ...
                                    <button @click="removeBlock(idx)" class="p-1 rounded text-gray-300 dark:text-slate-600 hover:text-red-500 dark:hover:text-red-400 transition-colors">
                                    ...
                                    <svg class="h-4 w-4 text-gray-300 dark:text-slate-600 ml-0.5 transition-transform" :class="expandedId === (block._id ?? idx) ? 'rotate-180' : ''"
                                        fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="m19.5 8.25-7.5 7.5-7.5-7.5" /></svg>
                                </div>
                            </div>
                            ...
                            <div v-if="expandedId === (block._id ?? idx)"
                                    ...
                                    <div class="grid grid-cols-2 gap-3">
                                        ...
                                        <div>
                                            ...
                                            <input v-if="block.duration_mode === 'distance'"
                                                v-model.number="block.distance_m" type="number" min="0" step="100"
                                                class="w-full rounded-xl border border-gray-200 dark:border-slate-700 bg-gray-50 dark:bg-slate-800 px-3 py-2 text-sm text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-indigo-500"
                                                placeholder="1000" />
                                            <input v-else
                                                :value="secToHms(block.duration_sec)"
                                                @change="block.duration_sec = hmsToSec($event.target.value)"
                                                type="text" pattern="\d{1,2}:\d{2}(:\d{2})?"
                                                class="w-full rounded-xl border border-gray-200 dark:border-slate-700 bg-gray-50 dark:bg-slate-800 px-3 py-2 text-sm text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-indigo-500"
                                                placeholder="10:00" />
                                        </div>
                                    </div>
                                ...
                                <template v-else-if="block.type === 'ramp_up' || block.type === 'ramp_down'">
                                    <p class="text-xs text-gray-500 dark:text-slate-400">Jede Stufe hat eine Zone und Dauer.</p>
                                    <div class="space-y-2">
                                        <div v-for="(step, si) in block.steps" :key="si"
                                            ...
                                            <input :value="secToHms(step.duration_sec)"
                                                @change="step.duration_sec = hmsToSec($event.target.value)"
                                                type="text" placeholder="03:00"
                                                class="w-20 rounded-xl border border-gray-200 dark:border-slate-700 bg-gray-50 dark:bg-slate-800 px-2 py-1.5 text-sm text-center text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-indigo-500" />
                                ...
                                <template v-else-if="block.type === 'repeat'">
                                    <div>
                                        <label class="block text-[11px] font-semibold text-gray-500 dark:text-slate-400 uppercase tracking-wide mb-1">Wiederholungen</label>
                                        <input v-model.number="block.repetitions" type="number" min="1" max="20"
                                            class="w-24 rounded-xl border border-gray-200 dark:border-slate-700 bg-gray-50 dark:bg-slate-800 px-3 py-2 text-sm text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-indigo-500" />
                                    </div>

                                    <div class="space-y-3">
                                        <div v-for="(step, si) in block.steps" :key="step._id ?? si"
                                            class="rounded-xl border border-gray-100 dark:border-slate-700 p-3 space-y-2">
                                            <div class="flex items-center gap-2 justify-between">
                                                <select v-model="step.type"
                                                    class="rounded-xl border border-gray-200 dark:border-slate-700 bg-gray-50 dark:bg-slate-800 px-2 py-1 text-xs text-gray-900 dark:text-white focus:outline-none">
                                                    <option value="work">Arbeit</option>
                                                    <option value="rest">Pause</option>
                                                    <option value="float">Float</option>
                                                </select>
                                                <input v-model="step.label" type="text" placeholder="Bezeichnung"
                                                    class="flex-1 rounded-xl border border-gray-200 dark:border-slate-700 bg-gray-50 dark:bg-slate-800 px-2 py-1 text-xs text-gray-900 dark:text-white focus:outline-none" />
                                                <button @click="removeRepeatStep(block, si)" :disabled="block.steps.length <= 2"
                                                    class="p-1 rounded-lg text-gray-300 dark:text-slate-600 hover:text-red-500 dark:hover:text-red-400 disabled:opacity-20">
                                                    <svg class="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M6 18 18 6M6 6l12 12" /></svg>
                                                </button>
                                            </div>
                                            <div class="grid grid-cols-2 gap-2">
                                                ...
                                                <input v-if="step.duration_mode === 'distance'"
                                                    v-model.number="step.distance_m" type="number" min="0" step="100"
                                                    class="rounded-xl border border-gray-200 dark:border-slate-700 bg-gray-50 dark:bg-slate-800 px-2 py-1.5 text-xs text-gray-900 dark:text-white focus:outline-none"
                                                    placeholder="m" />
                                                <input v-else
                                            ...
                                            <div class="grid grid-cols-2 gap-2">
                                                ...
                                                <input v-model="step.pace" type="text" placeholder="5:30 (optional)"
                                                    class="rounded-xl border border-gray-200 dark:border-slate-700 bg-gray-50 dark:bg-slate-800 px-2 py-1.5 text-xs text-gray-900 dark:text-white focus:outline-none" />
                                            </div>
                                        </div>
                                    </div>
